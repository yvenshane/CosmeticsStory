...
- (void)createSubViews{
    ...
    if (self.pageControlIsShow) {
        _pageControl = [[VENHomePageHeaderViewPageControl alloc]initWithFrame:CGRectMake(0, self.frame.size.height - 26, self.frame.size.width, 26)];
    ...
    for (int i = 0; i < (self.views.count - 1) / self.maxCount + 1; i++) {
        ...
        if ((self.views.count - i*5) <self.maxCount) {
...
//            NSLog(@"btnHieght = %f",(bgView.frame.size.height / 2));
            ...
            btn.frame = CGRectMake(col * (self.frame.size.width / self.lineMaxCount), row * (bgView.frame.size.height), (self.frame.size.width / self.lineMaxCount), (bgView.frame.size.height));
